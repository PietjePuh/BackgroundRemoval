...
def process_image(image_bytes):
    ...
    try:
        ...
        # Check for allowed formats to prevent processing of potentially malicious files
        if image.format not in ["JPEG", "PNG"]:
            print(
                f"Security Warning: Attempted to process disallowed image format: {image.format}"
            )
            st.error(
                "Unsupported image format. Please upload a PNG, JPG, or JPEG image."
            )
            return None, None
