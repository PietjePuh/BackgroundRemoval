...
def test_download_button_ux_label_and_help():
    ...
    result_bytes = b"a" * 1024  # 1 KB
    output_format = "PNG"

    bg_remove.display_single_result(
        mock_img, mock_result, output_filename, result_bytes, output_format
    )
    ...
    # The actual code now uses f"Download {output_format} image ({size_str})"
    ...
    assert "Download fixed.png" in help_text
